...
/** World-space quads for every explored cell; rehydrates the GPU mask from CPU memory. */
export function buildExploredCellQuads(grid: ExplorationGrid): Float32Array {
  const verts: number[] = [];
  for (let r = 0; r < grid.rows; r++) {
    const y = grid.oy + r * grid.cellSize;
    for (let c = 0; c < grid.cols; c++) {
      if (grid.cells[r * grid.cols + c] === CELL_UNEXPLORED) continue;
      const x = grid.ox + c * grid.cellSize;
      const x2 = x + grid.cellSize;
      const y2 = y + grid.cellSize;
      verts.push(x, y, x2, y, x, y2, x, y2, x2, y, x2, y2);
    }
  }
...
export class FogOfWarPass {
  ...
  public stampFowExploration(
    ...
    const stampMatrix = buildFowStampMatrix(
      fboManager.fowWidth,
  ...
  public restoreFromExplorationGrid(fboManager: FramebufferManager, grid: ExplorationGrid): void {
    ...
    gl.uniformMatrix3fv(gl.getUniformLocation(this.fowStampProg, 'u_matrix'), false, stampMatrix);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.dynamicBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, quads, gl.STREAM_DRAW);
