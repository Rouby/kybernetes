...
- Live inside the ship while it flies itself — watch cargo math, discover things out the windows.
...
- Phases: `docked → in_transit (guidance-predicted leg times) → docking (DOCKING_S=0s, instant on arrival) → docked`. No dead air around flights; plotting commits straight into transit.
- Departure gates on the full first-hop fuel cost but subtracts nothing; the bunker drains continuously in flight at `fuelRateForLeg(tier, thrust)` per real second. Legs fly at full rate; dry bunkers flame out.
- The torch is always hot: no spool charge, no tune decay, no wear. The reactor self-trims; only overheat warnings and scrams interrupt.
...
- Instant light-off: reactor must stay hot/powered + ≥1 fuel to commit, checked the same tick. Committed legs fly (DISTRESS is the way home).
...
- **Mid-leg drama = reactor drift only.** The plant self-trims but flux wander can still warn and scram; neglect triggers the existing scram → damage path. Wear/brownouts, combat/boarding, and power-triage spikes are explicitly out of v1.
...
interface VoyagePlan { hops: ChartHop[]; totalS: number; fuelNeeded: number; unknowns: string[]; destId: string }
```
...
- `legS` per hop = guidance-clocked `planTripLeg` (lane-table nominal fallback), so direct-hop projection matches the leg machine.
- `fuelNeeded` = summed per-hop burn-rate cost, no reserves.
...
- `fuelNeeded` comes from `planVoyage` single-hop projection — the web model no longer duplicates sim fuel rules.
...
- Wording lives in `navConsoleModel.fuelWarning` (docked-only); rendering lives in `UiScreens.navTextsFor` as a `warning`-color row above FLAMEOUT.

Examples:

| holds | tier | need | panel |
| --- | --- | --- | --- |
| 1 | 0 | 1 | no warning |
| 0 | 0 | 1 | `LOW FUEL: NEED 1 HOLD 0` |

## 7. Open questions

1. Long-haul fill: does survival decay alone carry a 10-min leg, or do we need timed contracts to force the pace?
