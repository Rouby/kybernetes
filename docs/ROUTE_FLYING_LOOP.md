...
1. - [x] 1a: chart graph + `planVoyage` projection helper (`sim-core`), fuel/heat warnings sourced from it (`web`).
   - [x] 1b: multi-segment `NavState` (`stops`/`legIndex`) + per-hop fuel burn (execution). `plotChartCourse`/`plotChartVoyage` commit chains; `NAV_STATE` carries `stops`/`legIndex`; nav panel shows the live `HOP` row on chains.
2. - [x] Waypoint intents: `NAV_PLOT` carries `waypointIds` (shape-checked, stale clients default to direct); the server commits via `plotChartVoyage`. The docked nav panel offers one `VIA <POI>` button per chart POI (phase-conditional, so the 4-button visor layout never overflows).
   - [x] Chart broadcast + manifest: `CHART_STATE` snapshots per vessel (daemon ships them with nav state); the nav panel shows discovery, best haul, and the live lane.
3. Nav console route-line + next-hop ETA (`web`; fuel projection + heat warning + HOP lane done).
   - [x] Two-step commit: clicking a hub/POI drafts a preview (ghost arcs on the map plus `PLAN <via><dest>`, `TIME <s>`, `FUEL NEED/HAVE` in the legend) and `CONFIRM` flies it; `CLEAR`/close/re-pick abandons the draft. Drafts live in console panel state, never on the wire.
   - [x] Big chart screen: opening the nav console shows the explored universe — central star, hubs/POIs as planets on orbit rings, plotted route polyline, live ship marker, fogged `??` nodes until surveyed. Click a hub to plot direct, a POI for the detour; legend card keeps PORT/DEST/ETA/FUEL/COUNTDOWN, warnings, and manifest rows.
   - [x] Physics transfer plot: sim leg timers are flown, not tabled: planTrip integrates each hop at the voyage torch setting and falls back to the lane table only when guidance cannot arrive, with the live total on the wire as `legTotalS`. The live leg stays a closed-form fixed-time rendezvous solved in the heliocentric frame — continuous burn out, flip, continuous brake, arriving with the target's position AND velocity (proven in trajectoryModel tests against the textbook rest-to-rest brachistochrone). Bodies carry state vectors on their circular orbits; the ship keeps its planet's heliocentric velocity at departure and retains velocity across legs and frames — nothing resets, nothing is animated. A sim-core astro module owns the system (one star mu, Kepler periods, exact circular kinematics) plus the rendezvous law with exact gravity cancellation. Future legs and ghosts fly planned guidance with slingshot gates around the star. Thrust lives on the chevrons as solved vectors: boost burn at the chain start (cyan + plume), brake burn at the predicted arrival (amber), flip diamonds where each leg reverses, burn-time ticks, `BURN 34S RETRO 33S` plan row. The flip point steers around the star across a candidate sweep (exact rendezvous holds at any flip time); selection prefers the smallest steer away from the balanced mid-flip among paths clear of the star body, falling back to deepest clearance — never a giant loop. While adrift, the committed route dims and a live amber intercept solves from the drift state. The ship marker integrates forward from a per-leg departure snapshot on the sim clock while the displayed route re-solves live from the ship — recomputed geometry never moves the marker, so it cannot jump; hop handoffs re-anchor onto the previous arrival and docking blends onto the live dock mouth. Broadcast clocks interpolate client-side between ~10Hz snapshots (sim time advances on the render clock, leg countdown with it; both freeze on pause, countdown also on flameout), so planets, routes, and the marker move constantly instead of stepping. The transfer row counts down across the flip (boost remaining, then brake remaining) instead of a fixed split.
4. - [x] Stranded triage: `hailS` countdown + refuel resume in the leg machine, `HAIL` intent (nav-console gated), stranded panel set (`HAIL RESCUE`/`DISTRESS`/`CLOSE`) with `DRONE IN <s>` row, DISTRESS tow kept as the paid fallback.
