...
export class AtmosphereSimulation {
  ...
  private calculateRoomDragForce(
    ...
    for (const flow of activeFlows) {
      const wind = portalWindAt(flow, target.position, room.averageMolarMass, density, soundSpeed);
      netWind.x += wind.x;
      netWind.y += wind.y;
    }

    // 3+4. Relative wind into aerodynamic drag
    const { force, dynamicPressure } = dragForceFromWind(
      density,
      cd,
      target.projectedArea,
      netWind.x - targetVel.x,
      netWind.y - targetVel.y
    );
    return { force, windVelocity: netWind, dynamicPressure };
  }
...
/** Bulk-gas properties driving drag: density (kg/m^3) and local Mach 1 (m/s). */
export function dragGasProperties(room: Room): { density: number; soundSpeed: number } {
...
export interface PortalWindInput {
  portal: Portal;
  molarRate: number; // mol/s
  isOutflow: boolean;
}
...
/**
 * Wind vector one active portal induces at a point (sink/source model).
 * Far field spreads hemispherically; the near field blends to the portal
 * orifice area so the singularity at r=0 never explodes, and the throat
 * caps at local Mach 1. Outflow pulls toward the portal, inflow pushes away.
 */
export function portalWindAt(
  flow: PortalWindInput,
  target: { x: number; y: number },
  molarMass: number,
  density: number,
  soundSpeed: number
): { x: number; y: number } {
  const { portal, molarRate, isOutflow } = flow;
  if (molarRate <= 0 || portal.effectiveArea <= 0) return { x: 0, y: 0 };

  // Volumetric flow rate through portal: Q = (n_dot * M) / rho (m^3/s)
  const volumetricRate = (molarRate * molarMass) / Math.max(1e-6, density);
  ...
  const localSpeed = Math.min(volumetricRate / flowCrossSection, soundSpeed);
  const dirSign = isOutflow ? 1 : -1;

  if (distance > 1e-4) {
    // Outflow pulls gas toward portal; inflow pushes gas away into the room
    return { x: (dx / distance) * dirSign * localSpeed, y: (dy / distance) * dirSign * localSpeed };
  }
  // Entity is at portal location, distance = 0: blow along the portal normal
  const ux = portal.side === 'east' ? 1 : portal.side === 'west' ? -1 : 0;
  const uy = portal.side === 'south' ? 1 : portal.side === 'north' ? -1 : 0;
  return { x: ux * localSpeed * dirSign, y: uy * localSpeed * dirSign };
}

/** Aerodynamic force from relative wind: F = 0.5 * rho * v_rel^2 * Cd * A. */
export function dragForceFromWind(
  density: number,
  dragCoefficient: number,
  projectedArea: number,
  relVx: number,
  relVy: number
): { force: { x: number; y: number }; dynamicPressure: number } {
  const relSpeedSq = relVx * relVx + relVy * relVy;
  const relSpeed = Math.sqrt(relSpeedSq);
  const dynamicPressure = 0.5 * density * relSpeedSq;
  const forceMagnitude = dynamicPressure * dragCoefficient * projectedArea;

  if (relSpeed <= 1e-5) return { force: { x: 0, y: 0 }, dynamicPressure };
  return {
    force: { x: forceMagnitude * (relVx / relSpeed), y: forceMagnitude * (relVy / relSpeed) },
    dynamicPressure,
  };
}
